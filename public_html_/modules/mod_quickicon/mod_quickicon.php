...
if (!defined( '_JOS_QUICKICON_MODULE' ))
{
	...
	<div id="cpanel">
		...
		require_once( JPATH_BASE .DS.'components'.DS.'com_joomlaupdater'.DS.'core'.DS.'latestversion.php' );
		if (GetJoomlaLatestVersion() > JVERSION)
			$linkText = "<font color='red'><blink>Click to update Joomla</blink></font>";
		else
			$linkText = "No need to update !";

		$link = 'index.php?option=com_joomlaupdater';
		quickiconButton( $link, 'magic-logo.png', JText::_( $linkText ) );
		?>
	</div>
	<?php
}