...
class plgContentCkForms extends JPlugin
{
	...
	function generateCkFormHTML( $form, $fields)
	{
			...
			if ($field->typefield != "hidden" && $field->typefield != "button" && $field->typefield != "fieldsep")
			{
				...
				switch ($field->typefield)
				{
					...
					{	
        				...
        				$html = $html.'<textarea class="ckCSSinputnormal ckCSSnoTip" name="'.$field->name.'" id="'.$field->name.'" cols="'.$field->t_columns.'" rows="'.$field->t_rows.'" wrap="'.$field->t_wrap.'" ';
						if ($field->readonly == "1")
						{
							$html = $html.' readonly="true"';
						}
						
						$html = $html.'>'.$field->t_initvalueTA.'</textarea>'."\n";
		            }
					break; 	
		...
		if ($form->captcha == 1)
		{
			...
			$html = $html.'<img class="captchacode" src="index.php?option=com_ckforms&task=captcha&sid=c4ce9d9bffcf8ba3357da92fd49c2457" align="absmiddle"> &nbsp;'."\n"; 
			$html = $html.'<img alt="Refresh Captcha" class="captcharefresh" src="'.JURI::root(true).'/components/com_ckforms/captcha/images/refresh.gif" align="absmiddle"> &nbsp;'."\n"; 
			$html = $html.'<input type="text" id="ck_captcha_code" name="ck_captcha_code" />'."\n"; 
			
			if ($form->captchacustominfo != "") 
			{
				$html = $html.'<img class="ckform_tooltip" src="'.JURI::root(true).'/components/com_ckforms/img/info.png" title="'.$form->captchacustominfo.'" />'."\n";
			}		
